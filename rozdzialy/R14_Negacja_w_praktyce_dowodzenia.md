...
do uczenia się poprzez wcielenie się w rolę. Poza tym w niektórych miejscach wyjaśniam tu po raz
kolejny kwestie, o których już wcześniej mówiłem. Wiem, że ryzykuję w ten sposób, że zaczniesz się
(jeszcze bardziej) nudzić, ale gdyby tak się stało, to przynajmniej dzięki temu będziesz wiedzieć,
że pewne rzeczy już opanowałaś. A pod koniec tego rozdziału poznasz bardzo użyteczną *implikacją
przeciwną*.
...
1. W logice konstruktywnej negacja zdania jest tak naprawdę pewną szczególną implikacją. Jeżeli `p :
   ...
   dowolny dowód tego zdania przekształca w (czysto hipotetyczny, ale o tym później) *dowód
   fałszu*. A więc:
...
```lean
-- Zawsze, gdy mamy dowody pewnego zdania (tutaj `p`) i jego negacji, ...
variable (p : Prop)
variable (hp : p)
variable (np : ¬p)
...
negacji, albo na *użyciu* (dowodu) negacji. Może zaczniemy od użycia.
...
żadnej wewnętrznej struktury, a bez dodatkowych założeń takie zdania nie są ze sobą związane
logicznie.

Gdy nie ma dodatkowych przesłanek, które wiązałyby ze sobą - bezpośrednio lub pośrednio - jakieś dwa
zdania, mówimy, że takie zdania są *logicznie niezależne*. W ogólnym przypadku logiczna
(nie)zależność między zdaniami zależy więc od kontekstu.
...
definicji implikacją z `p` do fałszu, to powinniśmy aplikować dowód `¬p`, czyli term typu `p →
False`, do dowodu `p`, czyli do termu typu `p`, ale kiedy stosujemy funkcję `absurd` kolejność
termów jest odwrotna. No trudno, taka konwencja.
...
że `¬p → p → q` jest zdaniem. Jednocześnie `p` i `q` mają być *zmiennymi*, za które chcemy móc
podstawiać dowolne zdania stosując to twierdzenie w przyszłości jak każdą inną funkcję. W takim
razie "zmienno-zdaniowość" albo "zmiennościowość i zdaniowość" nazw `p` i `q` musi być zakodowana
poprzez to, że `p` i `q` odgrywają rolę parametrów dowodzonego twierdzenia. Możemy te parametry
umieścić przed dwukropkiem albo za, a jeżeli za, to możemy oznaczyć je za pomocą symbolu `∀`:

```lean
-- Dokończ dowód w trybie interaktywnym, nieinteraktywnym, lub w obu tych trybach. Jeżeli bez
-- dodatkowych wyjaśnień to dla Ciebie w tym momencie zbyt trudne, to moje próby pomocy zadania
-- znajdziesz niżej.
...
poprzednik, w ten sam sposób co wcześniej (tylko inaczej nazywając hipotezę), bo to też jest
...
wypadku opcjonalnego) argumentu. A nie będzie trzeba tego robić, bo gdy jesteś w trybie dowodzenia
(niekoniecznie interaktywnego), Lean "zakłada", że chcesz udowodnić aktualny cel, i sam "decyduje",
że eksplozja dedukcyjna ma być wykorzystana właśnie na rzecz tego celu.

Możesz spróbować skonstruować dowód w trybie nieinteraktywnym (a więc bez użycia taktyk `intro` czy
`exact`), konstruując funkcję dowodu `¬p`, zwracającą funkcję (inaczej nazwanego) dowodu `p`, która
...
widzisz jednoznaczny związek między tamtym dowodem, a tym, jak wygląda ten ostatni fragment kodu. Na
przykład, w trybie interaktywnym komenda `intro` z wybraną przez Ciebie nazwą zaczęła tworzyć kod
anonimowej funkcji o parametrze, który podałaś jako argument tej komendy.

Jak już wspominałem, w trybie nieinteraktywnym nie zadziałają komendy `exact`, `intro`, ani żadna
inna taktyka. Możesz jednak zawsze *w dowolnym miejscu dowolnej definicji* (nie tylko twierdzenia)
wejść w tryb interaktywny pisząc `by` i próbować zakończyć konstrukcję, a więc również jakikolwiek
dowód albo tylko jego fragment, używając taktyk.
...
spacji) `.elim` (z kropką na początku), zastosujesz tą regułę, co natychmiast zakończy dowód. A więc
ten sposób zadziała tak samo, jak zastosowanie taktyki `exact` do aplikacji funkcji `absurd` w
...
Chcesz, żebym to też omówił krok po kroku? Jeżeli (ilustracja) ...
...
Leanowi będzie wszystko jedno; oferuje dwie konwencje zamiast jednej dla Twojej wygody.

Podsumowując, gdy masz ochotę użyć jawnie reguły eliminacji fałszu, musisz mieć jakiś dowód
...
typów i izomorfizmu Curry'ego-Howarda to wszystko jest tym samym. Użyłem tym razem dużej litery `P`,
żeby zasygnalizować, że to może być jakieś zdanie złożone, a nie atomowe, na przykład jakaś
implikacja. Gdy masz już dowód fałszu, wystarczy, że powiesz Leanowi, że chcesz go zastosować do
skonstruowania dowodu zdania, które w danym miejscu "czeka na bycie udowodnionym". Byłoby chyba
najlepiej, gdybyś spróbowała zakończyć dowód twierdzenia `tn1` używając więcej niż jednego z
wymienionych sposobów, ale nie będę się przy tym upierał.

**Polecenie**: Spróbuj udowodnić to twierdzenie w wybranym przez siebie trybie. Wydaje się
trudniejsze, ale tak naprawdę tylko *wygląda* na bardziej skomplikowane. Być może pójdzie Ci
najłatwiej, jeśli wejdziesz w tryb interaktywny i zaczniesz *swobodnie eksperymentować* z komendami
`intro`, `exact`, funkcją `absurd` i aplikacjami. Jak się pogubisz, zawsze możesz usunąć fragment
zapisanego kodu i spróbować inaczej.

```lean
theorem tn2 (p : Prop) (q : Prop) : ¬(p → q) → (p → q) → q :=
```

No dobrze, teraz muszę jeszcze wymyślić zadanie, które będzie polegało na *uzyskaniu*
...
*samego fałszu*, a raczej możemy, ale zadanie byłoby wtedy trywialne, bo dałoby się udowodnić każde
...
Chyba wiem co zrobić. Zamiast jednego zadania zaproponuję Ci trzy.
...
Udowodnij więc poniższe twierdzenie, najpierw korzystając z trybu interaktywnego, a potem bez użycia
komendy `by`. Wystarczy wprowadzić poprzednik implikacji jako założenie i użyć tego hipotetycznego
dowodu fałszu wybierając jeden ze sposobów, które dotąd objaśniłem.
...
Jednak zdanie. Ciekawe? Moim zdaniem bardzo. Wrócimy do tego niebawem, gdy zaczniemy się posługiwać
[językiem kosmitów](https://en.wikipedia.org/wiki/Category_theory).
...
fałszu pozwoliłby udowodnić wszystko, co byśmy chcieli.
...
naturalnych, bo ten typ już znasz. Uprzedzam tylko, że być może trzeba będzie otoczyć niektóre
aplikacje predykatów nawiasami.

**Polecenie**: Udowodnij poniższe twierdzenie. To twierdzenie tylko wygląda na bardziej
skomplikowane; przekonasz się, że w dowodzie nie będziesz korzystać z faktu, że zdania mają
...
Co by tu napisać, żeby zmniejszyć ryzyko paniki... Może tak: Często parametry twierdzeń zawierają
tylko informacje o tym, co *znaczą* poszczególne symbole lub nazwy, a więc wystarczy na te parametry
rzucić okiem, żeby się zorientować, *o czym jest mowa*. W tym wypadku twierdzenie ma dotyczyć jakiś
predykatów, `P` i `Q`, i jakiejś liczby naturalnej `n`. To jest odpowiednik części *Jeżeli* n *i* m
*to liczby naturalne...*, z którą spotkałaś się w rozdziale czwartym. Właściwie to tylko [nudna
...
Być może męczy Cię to powtarzanie, ale nie mogę zakładać, że udało Ci się to już dobrze załapać:
Przypominam, że zapisując początek twierdzenia `tn3` musieliśmy wstawić parametr typu `Nat`, bo
inaczej nie moglibyśmy pisać zdań złożonych z predykatów dotyczących liczb naturalnych w sposób,
który byłby dla Leana "zrozumiały". Sam predykat, na przykład `Ponury`, nie jest zdaniem (jest
predykatem, a więc pewną funkcją {nie *typem* funkcyjnym}). Dopiero, dajmy na to, `Ponury Krystian`
jest zdaniem. Analogicznie, sam predykat `P : Nat → Prop`, dotyczący liczb naturalnych, nie jest
jeszcze zdaniem, dopiero aplikacja `P` do liczby naturalnej jest zdaniem. Skoro więc mamy dowodzić
zdania na temat (jakiś) własności (tutaj `P` i `Q`) liczb naturalnych, to musimy mieć do dyspozycji
co najmniej jedną, bliżej nieokreśloną (lepiej: *arbitralną*[^2], a może nawet "wirtualną"?) liczbę
naturalną, której te własności mogą przysługiwać albo nie.

Wewnątrz (w lokalnym kontekście) dowodu twierdzenia `tn3`, od samego początku są więc widoczne
*trzy* termy (to są właśnie parametry tego twierdzenia): (jakieś) dwa predykaty dotyczące liczb
naturalnych - `P` i `Q` - i (jakaś) liczba naturalna `n`. Wewnątrz dowodu zawsze możemy korzystać z
parametrów dowodzonego twierdzenia, bo twierdzenie to dla nas to samo co funkcja zwracająca term
typu `Prop`, a wewnątrz funkcji możemy zawsze korzystać z jej parametrów (o ile ich lokalnie nie
przysłonimy tworząc jakby "wewnętrzny kontekst" i nadając nazwom parametrów nowe znaczenie, ale o
tym kiedy indziej).

To wszystko komplikuje tylko strukturę *typu stałej* `tn3`, ale nie ma znaczenia dla *dowodu* tego
twierdzenia. Tak naprawdę mamy tu tylko implikację, której następnikiem jest implikacja, i której
poprzednikiem co prawda też jest (szczególna) implikacja (bo negacja to tak naprawdę implikacja),
ale tej negacji-implikacji nie musimy "rozbierać na części", bo pełni rolę przesłanki (to jest
poprzednika). Obie "zwykłe" implikacje musimy kolejno "rozłączać", wprowadzając (do kontekstu) jako
lokalne hipotezy ich poprzedniki, a raczej hipotetyczne dowody (czyli założenia) tych
poprzedników. Wreszcie występującą tu negację-implikację musimy tylko - jawnie lub niejawnie -
aplikować.
...
`¬q`, to nie może być prawdą, że `p`. Gdyby bowiem *wtedy* (gdy `¬q`) było prawdą, że `p`, to
moglibyśmy użyć `p → q` do uzyskania `q`, co byłoby sprzeczne z `¬q`.
...
**Polecenie**: Udowodnij teraz, albo kiedy indziej, albo wcale, jak uważasz, twierdzenie `cp` (to
skrót od [*contraposition*](https://en.wikipedia.org/wiki/Contraposition)). Użyłem tu krótszego
zapisu parametrów `(p q : Prop)` zamiast `(p : Prop) (q : Prop)`. Jak już wiesz, dla Leana to znaczy
to samo, ale pozwala na taki skrót tylko wtedy, gdy sąsiadujące parametry mają ten sam typ. Lubię
ten zapis.
...
W ciele (definicji tego) twierdzenia masz jakieś dwa zdania (`p` i `q`), ale z tego akurat będziesz
korzystać tylko "w tle". Ważniejszy jest jakiś dowód `h `implikacji `p → q`, którego również możesz
używać *bez* wprowadzania do kontekstu (za pomocą `intro`), bo też jest parametrem.
...
rozkładać nie będziesz. Przypuszczam, że największą trudność sprawi Ci ostatni krok, gdy celem
będzie `¬p`. Ponieważ `¬p` to tak naprawdę implikacja, trzeba będzie wtedy stworzyć odpowiednią
funkcję, co w trybie interaktywnym można zrobić za pomocą taktyki ...
