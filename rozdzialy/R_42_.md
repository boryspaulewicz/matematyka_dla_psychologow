...
popularnej papugi należącej do gatunku [LLM](https://en.wikipedia.org/wiki/Large_language_model),
następujące uwagi, które Ci autorzy zapisali w roku 98 i które już wtedy były tylko przypomnieniem
kwestii doskonale znanych specjalistom, powinny rozwiać przynajmniej niektóre Twoje wątpliwości:
...
Może warto tylko dodać, że LLM-y są (w pewnym sensie) swoimi własnymi nauczycielami, ale ta różnica
jest dość powierzchowna, bo tym, co ma tu największe znaczenie, jest *dostępność informacji na temat
poprawnych reakcji*, a nie to, czy proces uczenia (się) jest *zautomatyzowany*.
