...
czerwieni. I rozmaitego szumu, i to by było w zasadzie na tyle, bo przecież - powiedzmy to sobie
szczerze - "écru wpadające w limonkę" to nie tyle kolor, ile raczej stan wymagającego pilnej
psychologicznej interwencji, schorowanego umysłu, a ryba, jak wiadomo, jest właściwie
[warzywem](https://youtu.be/MPY_EuvimH0?si=XMt5bmJatCwwfAJs).

## Trzeci - też śmieszniejszy, ale to już jest mniej oczywisty żart - sposób na psychologię

Jednym z brzemiennych w poważne skutki zdarzeń, które przytrafiło mi się w trakcie dwóch lat
codziennego systematycznego uczenia się różnych rzeczy, o którym to okresie wspominałem we
...
Barto ([pierwsze wydanie](http://incompleteideas.net/book/the-book-1st.html) nie jest może tak
aktualne jak drugie, ale też jest wspaniale napisane i jest przy tym znacznie krótsze, a więc
szybciej wchodzi). 

Będzie chyba lepiej, jeśli zanim zacznę o tym gadać, oddam głos tym dwóm cudownym autorom i moim
zdaniem (z konieczności tylko lokalnie, ale jednak) głębokim, wytrawnym i diabelnie przenikliwym
myślicielom. Z obwoluty możemy się dowiedzić, że (cytaty pochodzą z wydania pierwszego) ...
...
... się chłopaki mocno przeliczyły. Bo niestety większość współczesnych psychologów zajmuje się
uprawianiem nauki [performatywnej](https://en.wikipedia.org/wiki/Cargo_cult), ktora pasie się na
ogól z dala od technicznie złożonych tekstów i autentycznego, systematycznego uczenia się właściwie
czegokolwiek. Ale miałem tyle nie gadać, przepraszam.

Już nic nie mówię.

Normalnie zamieniam się w Króla Ciszy.
...
że siedzimy lub stoimi przed takim sporym pudłem, ciągniemy za wystającą z prawej strony wajchę, po
czym patrzymy na zmieniające się w kilku okienkach obrazki i rujnujemy sobie i swoim ewentualnym
bliskim życie. Co ciekawe, wiele osób wydaje się sądzić, że adekwatnym określeniem dla tej dualnie
...
4. posługując się dowolną relacją porządku (`[Porzadek Sygnal]` \{przypominam, że to jest powołanie
...
nic innego do roboty, `n = 3`, a nagrodami, jak to w kasynie, są wypłaty pieniężne, które mimo
swojej dosyć szczególnej sytuacji nadal sobie cenisz. Załóżmy, że zaczęłaś od jednokrotnego
sprawdzenia każdego z dostępnych działań:

```lean
-- Ten parametryczny typ pozwala wygodnie rozumować na temat czegoś, co może być niedostępne albo
-- niepoznawalne albo bezużyteczne. W Leanie ten typ nazywa się `Option` i ma konstruktory o nazwach
...
Przypuszczam, że sposobu działania wyrażeń o postaci `if <warunek> then <term> else <term>` nie
muszę tłumaczyć, może poza jedną subtelnością: Żeby Lean takie coś zaakceptował, term `warunek` musi
być *zdaniem rozstrzygalnym*, co znaczy, że Lean musi być w stanie automatycznie znaleźć dowód
...
jeden stan tego (zewnętrznego wobec nas?) świata. A stąd wynika, że w tym świecie wszystko działa
zawsze tak samo, niezależnie od tego, co się wcześniej wydarzyło.
...
No więc wyobraź sobie, że na początku zastosowałaś `n` razy tą *strategię eksploracyjną*, uzyskując
takie oto (funkcjonalnie) zyski, będące jednocześnie (funkcjonalnie) jedynymi dostępnymi
informacjami na temat dynamiki tego środowiska (jako zadania):

`0 ↦ 665, 1 ↦ 664, 2 ↦ 667`

W tym wypadku możemy jednoznacznie i autorytatywnie stwierdzić, co *powinnaś* zrobić, bo to wynika z
definicji, a wynika z niej dokładnie dlatego, że to jest definicja *zadania*. Stwierdzenie, że coś
się powinno albo że czegoś się nie powinno ma sens *tylko* zakładając jakiś cel, normę, albo
wartość, czyli ostatecznie jakiś cel, a ponieważ cel jako taki jest zawsze pewną preferencją, czyli
oceną, a ocena jako taka jest zawsze postawą, funkcjonalnie równoważną pewnej relacji porządku,
czyli ostatecznie punktem widzenia, założenie na temat celu jest
[*nieuzasadnialne*](https://pl.wikipedia.org/wiki/Gilotyna_Hume%E2%80%99a) inaczej niż ze względu na
jakieś inne cele (czyli pewne preferencje, postawy, oceny, wartości, czy normy i zarazem -
formalnie - relacje porządku):

*Jeżeli* zależy Ci tylko na pieniądzach (*ocen jako takich* nie oceniam), to zgodnie z wszystkimi
dostępnymi danymi, *powinnaś* wybrać `2`, ponieważ *najlepszym*, albo *racjonalnym*, albo
*poprawnym*, a może nawet *jedynym sensownym* wyborem jest *w tej sytuacji* `2` i tylko `2`.
...
aktualnego stanu* (świata), inaczej *sygnału*, ...

2. działania - a właściwie lepiej *ruchu* - wykonanego w danej *próbie* i ...

3. stanu *całego* (ale za to tylko rozważanego, a więc może być przynajmniej subiektywnie mały)
   *świata*.

Zwracam też uwagę, że mówimy tu o środowisku, celach, sygnałach stanu i reakcjach *z perspektywy*,
potencjalnie czysto hipotetycznej, *wszechwiędzącej obserwatorki*.

## ImPeRaToRR

Zmienimy teraz język na taki, który [wymaga](https://pl.wikipedia.org/wiki/Odzie%C5%BC_ochronna)
założenia co najmniej rękawic do smaru i oleju. I okularów ochronnych. Szczerze mówiąc, maska
przeciwpyłowa też by nie zaszkodziła. Będzie to mianowicie używany przez wielu psychologów
zajmujących się (często niestety nieudolnie) analizą danych język imperatywny R. 
...
*potencjalnie* (bo to, czy można takie ruchy nazwać decyzjami, zależy od tego, jak to wszystko razem
działa) decyzję, której wartościami mogą być liczby naturalne od 0 do 2. W ciele tej funkcji,
...
wskazywania elementu sekwencji za pomocą liczby naturalnej, którą to operację oznaczamy również
między innymi w języku R używając nawiasów kwadratowych. Ponieważ jednak w R indeksujemy elementy
należące do sekwencji zaczynając od 1, a nie - jak w tych bardziej cywilizowanych językach
programowania - od 0, do indeksu działania dodajemy najpierw 1.

R jest (interpretowanym) językiem imperatywnym, dlatego `E` to tutaj *zmienna*, która będzie nazwą
funkcji środowiska dopiero od momentu, w którym poddamy ten fragment kod ewaluacji. W
przeciwieństwie do Leana, napisany w skrypcie R-a kod nie zaczyna więc "od razu działać". Widzimy
też, że nie ma tu żadnej jawnej informacji o typach danych, bo (ubogi) system typów R-a został
zaprojektowany do realizacji znacznie skromniejszych celów niż formalizacja całej matematyki.
...
styl kodowania często stosowany w językach funkcyjnych, a nie imperatywnych:
...
## To jest odtąd nasz (kolejny) model środowiska, ale rozumianego jako zadanie, a więc środowiska w
...
## A to jest funkcja "podmiotki", inaczej "agentki". Przyjmujemy tutaj, że ta agentka na
## początku nie odbiera żadnego sygnału ze środowiska (`sygnal = NA`), jej pamięć jest pusta,
## kondycja ma poziom wyjściowy, i jest najmłodszą możliwą wersją siebie.
...
A = function(sygnal = NA, ## To są domyślne wartości parametrów tej funkcji, ...
    ...
    zycia = zycia - 1
    ## ... aplikujemy (funkcję) Się do dostępnych danych:
...
Cały ten kod możesz wkleić na przykład [tutaj](https://rdrr.io/snippets/), albo do zainstalowanego
na swoim komputerze interpretera języka R. Po wykonaniu ewaluacji (albo po *u-ruchomieniu*) tego
kodu przekonasz się, że - jak można policzyć (ale komu by się chciało) - po pierwszych dwóch
wyborach eksploracyjnych ta agentka wybiera już do samego końca ruch, po którym występuje największy
sygnał nagrody:
...
wejść. Wynika stąd, że funkcje w R zachowują się często jak procesy *fizyczne*, a nie jak "zwykłe"
"procesy" *logiczne* (inaczej matematyczne).

Kod napisany w R to jednak mimo to *również* matematyka. *Możemy* dowodzić twierdzeń dotyczących
sposobu działania kodu napisanego w językach imperatywnych, bo te języki też są formalne, a to
znaczy, że napisane w nich programy działają zgodnie z jednoznacznymi, sztywnymi zasadami, a zatem
zgodnie z jakąś logiką. Różnica polega na tym, że dowodzenie twierdzeń na temat działania programów
napisanych w językach imperatywnym może wymagać uwzględnienia modelu komputera kontrolowanego przez
tego rodzaju programy, a czasem nawet pewnego modelu świata, z którym wykonujący je komputer wchodzi
w interakcję. A to wymaga zastosowania logiki *przyczynowej* (którą poznamy kiedy indziej), co też
nie powinno nikogo dziwić, bo w określeniu "skutek uboczny" jest przecież, jak byk (ale w dobrym
sensie) słowo (sąsiadujące ze słowem) "uboczny". 

To ja już może lepiej odprowadzę się do drzwi.
...
R jest standardowym oznaczeniem braku danych (ang. *Not Available*). Czyli w dodatku ta agentka
"rodzi się z zamkniętymi oczami".

Mogłoby się więc wydawać, że rodzi się jako *tabula rasa*, ale to tylko złudzenie, wynikające z
naiwnego, wąskiego rozumienia takich pojęć jak *pamięć* i *wiedza*. Pamięć jest tu co prawda
...
albo większym *tłumie*, zwłaszcza gdy te przebywają na tyle blisko siebie, by móc się łatwo
komunikować. 
...
No dobrze, teraz już mogę oddać głos autorom.
...
niezwykle szkodliwej, ale bardzo ostatnio popularnej i chętnie kupowanej papugi, należącej do
gatunku [LLM](https://en.wikipedia.org/wiki/Large_language_model), następujące uwagi, które Ci
autorzy zapisali w roku 98, a które już wtedy były tylko przypomnieniem kwestii doskonale znanych
...
Może warto tylko dodać, że LLM-y są (w pewnym sensie) *swoimi własnymi* nauczycielami. Ta różnica
jest jednak powierzchowna, bo tym, co ma tu - czyli dla *rozwiązania jakiegoś problemu* - znaczenie
kluczowe, jest *dostępność informacji na temat poprawnych reakcji*, a nie to, czy proces uczenia
(się) jest *zautomatyzowany*.
...
dobra jest ta teoria, rozważymy nieco inny problem[: ...](./R_42_3.md)
