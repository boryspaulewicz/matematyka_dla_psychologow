...
warunek poprawności definicji funkcji, która wymaga argumentu typu `Empty`, wystarczy skorzystać z
reguły eliminacji dla tego typu. W szczególności, nie musimy nawet konstruować drugiej, wewnętrznej
funkcji tworząc term typu `Binrel Empty Empty`, czyli typu `Empty → Empty → Prop`, bo ta reguła
eliminacji pozwala od razu "spełnić oczekiwanie" stworzenia termu dowolnego typu, w tym również typu
`Empty → Prop`, za pomocą czegoś w rodzaju, w tym wypadku akceptowalnej, "pustej obietnicy", albo
"obietnicy* *z konieczności* "bez pokrycia".
...
poprawne\}. W dodatku ten głęboki związek między celowością i logiką nie jest wcale
przypadkowy. 
...
*różnica*. A gdy *jedynymi* dwoma obiektami są wzajemnie dualne obiekty początkowy i końcowy,
istnieje dokładnie jedna strzałka. W matematyce, tak jak w psychologii, wszystko sprowadza się
...
zdaniem doskonałe pytanie, które okazuje się być zaskakująco głębokie. Dlatego w tym momencie powiem
tylko tyle, że w pewnym sensie *po niczym*.
...
*mentalne*, polegają na poruszaniu się głównie *wewnątrz ciała*. Jeżeli to uogólnienie Ci nie
odpowiada, to zwróć proszę uwagę, że nie jest wewnętrznie sprzeczne, a wobec tego, jak każdy *wybór
pojęciowy*, podlega ocenie *tylko* ze względu na to, do czego się przydaje. 

Warto dokonać tego uogólnienia, bo dzięki niemu można zobaczyć wyraźnie coś, co bardzo długo umykało
mojej uwadze. Pouczający jest też powód, dla którego tak długo nie udawało mi się tego
dostrzec. Byłem mianowicie od dawna przekonany, że istnieje jakaś fundamentalna różnica między
*zachowaniami zwierząt* z jednej strony, a *sposodem działania urządzeń* i wszelkiego rodzaju
*narzędzi* z drugiej. Wydawało mi się na przykład, że ta różnica polega na *wyuczalności*
zachowania, z której wynika jego wyjątkowa i adaptacyjna elastyczność i której nie widać w typowych
urządzeniach, narzędziach, czy wszelkiego rodzaju mechanizmach. 

Proszę, nie zrozum mnie źle. Nie uważam, że zwierzęta po prostu *są* urządzeniami. Na pewno nie *dla
nas*, bo *współczujemy* zwierzętom, natomiast nie bardzo współczujemy urządzeniom. A gdy czasem do
pewnego stopnia współczujemy niektórym urządzeniom, zwłaszcza tym, do których jesteśmy w jakiś
sposób *przywiązani*, mamy do takich reakcji pewien dystans, którego nie mamy w stosunku do własnego
współczucia wobec zwierząt. Zwierzęta zdecydowanie *nie* są więc (dla nas) po prostu urządzeniami bo
(z naszej ludzkiej perspektywy) zwierzęta *zasługują na współczucie i szacunek*. 

Ale **zachowanie**, ten centralny przedmiot badania psychologii, to dokładnie *działanie celowe*,
albo, po dokonaniu uogólnienia już równoważnie, *celowy ruch*; nic mniej i nic *więcej*. Własność
wyuczalności przysługuje pewnym celowym ruchom, a dokładniej pewnym *sekwencjom ruchów*, co sprawia,
że te ruchy są automatycznie szczególnie interesujące, a często wręcz przykuwają naszą uwagę i
automatycznie widzimy w nich coś w rodzaju *autonomii* (pamiętasz tą mysz?); ale celowość występuje
w sposób oczywisty również we wszystkich nie uczących się narzędziach czy urządzeniach.
