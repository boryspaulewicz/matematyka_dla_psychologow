...
przykuwają naszą uwagę i automatycznie widzimy w nich coś w rodzaju autonomii (pamiętasz tamtą
mysz?); ale celowość występuje w sposób oczywisty również we wszystkich nie uczących się
narzędziach, urządzeniach, mechanizmach i wszelkich innych artefaktach, w tym również w dziełach
sztuki.
...
proszę mnie poprawić), czemu nie można o tym myśleć jako o *spadaniu* w czasie? Wtedy czas płynąłby
w górę, a my w dół. Pamiętasz dualność relacji `<` i `>`?
...
taki Einstein już dawno to załapał, tyle, że *mniej ogólnie*. Można o tym myśleć też tak: nad każdym
naszym ruchem jakby unosi się (jako mnogość punktów widzenia) nieskończenie złożona przestrzeń
najróżniejszych celów (które zawsze można traktować jako pewne relacje porządku) i w każdym momencie
każdego ruchu i dla każdego celu istnieje miara stopnia, w jakim aktualne położenie i struktura
wewnętrzna naszego ciała realizuje albo spełnia ten cel.
...
czas można uporządkować na nieskończenie wiele, również nieizomorficznych sposobów. Na przykład,
można uporządkować nieskończony zbiór punktów czasowych ze względu na stopień zagęszczenia materii
we Wszechświecie, albo można obrać jakiś punkt zerowy na skali czasu i wtedy czas co prawda nie
...
Mamy tu kategorię `Pracownicy`. Na tym diagramie, poprzedzonym czymś w rodzaju "preambuły"
definiującej typowanie - a więc i tu pomieszałem dwa różne języki, z których jeden właśnie
wymyśliłem - strzałka oznacza bezpośrednie zwierzchnictwo:
...
Czuję, że teraz już muszę powiedzieć więcej o samej terminologii. Relacje (`R ⊆ X × X`) zwrotne (`∀
x : X, x R x`) i przechodnie (`∀ x y z : X, x R y → y R z → x R z`), ale *niekoniecznie*
antysymetryczne, nazywamy *praporządkami*, a po angielsku (spolszczając) *preorderami*. Praporządki
to dokładnie cienkie kategoria, w których mogą występować pary różnych obiektów izomorficznych, albo
mówiąc równoważnie, kategorie cienkie, w których występują strzałki przeciwrównoległe nie będące
identycznościami. Nie mam pojęcia, który sposób myślenia jest dla Ciebie w tym momencie
najwygodniejszy, ale całkiem możliwe, że *rysunkowy*, dlatego zachęcam do prób oswojenia się z tą
terminologią (bo to jest niemal całkowicie tylko gęsta terminologia dotycząca bardzo prostych
zjawisk, którą trzeba tylko w miarę dobrze zapamiętać) za pomocą jakiejś częściowo rysunkowej
notatki.
...
nie ma końca. Porządek liniowy to poset, w którym każda para elementów jest *porównywalna* (`∀ x y :
X, x R y ∨ y R x`). W ogólnym przypadku zarówno preordery jak i posety mogą się składać z
*oddzielnych* [drzew](https://pl.wikipedia.org/wiki/Drzewo_(informatyka)) (rozumianych jako pewien
typ danych), przy czym preordery mogą mieć różne elementy połączone dwukierunkowo "w poziomie", a
posety nie.

**Sugestia**: Zastanów się może, w jaki sposób mogłabyś zacząć próbować skonstruować dowód tego
twierdzenia. Podkreślam, że nie sugeruję, żebyś je udowodniła, chociaż jestem przekonany, że
zrobiłabyś to bez większego trudu o ile udałoby Ci się zachować odpowiednie nastawienie. Chodzi mi
raczej o próbę metarefleksji na temat własnych sposobów myślenia. Korzystam w tym kodzie z
przytoczonych wcześniej definicji.

```lean
-- Wprowadzam tu uproszczony sposób pisania, odpowiadający często stosowanemu uproszczonemu
...
-- Jeżeli przyszło Ci do głowy, że może warto byłoby to zakodować za pomocą klas typów, to
-- brawo. Faktycznie byłoby warto, ale to dla naszych skromnych celów ta wersja nam wystarczy.

-- Element najmniejszy *ze względu na relację `R`* to taki `x`, że dla każdego `y`, `x R y`, a w
-- notacji przedrostkowej:
def least x := ∀ v, R x v

-- Zwracam uwagę, że definicja elementu najmniejszego *nie* zakłada, że dana relacja jest w
-- jakimkolwiek sensie relacją porządku. Ta relacja może być *dowolna*.
