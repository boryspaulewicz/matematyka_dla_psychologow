...
nowego języka*, który pozwalałby w jednoznaczny, zwięzły i zarazem możliwie czytelny sposób
zapisywać różne definicje. Wydaje się, że zależałoby Ci wtedy na zapisaniu informacji, która
pozwalałaby określić, jakie operacje z użyciem definiowanych stałych mają sens, a jakie go nie
mają. Byłoby też wskazane, żeby nie używać do tego symbolu równości (*=*), bo ten odgrywa a
matematyce bardzo ważną rolę i mógłby się jeszcze później przydać do całkiem innych celów, to jest
do oznaczania "zwykłej" relacji równości, a nie *decyzji*, że jakieś wyrażenie ma być wartością
jakiejś stałej.
...
powiedziała na takie oto rozwiązanie?
...
-- termy (jak również stałe) oznaczające różne typy
#check (dziesiec : String)
```

Wiesz już teraz, jak możesz definiować nowe stałe oznaczające liczby naturalne lub wartości
...
tych rzeczy, zawsze będziesz mogła to sobie szybko przypomnieć zaglądając do tego rozdziału.
...
Pozostaje jeszcze kwestia sposobu zapisywania typów tego rodzaju wyrażeń (a dokładnie takich *termów
złożonych*). Zgodzisz się chyba, że naturalnie byłoby oznaczać je w sposób, który mówi jaki jest typ
argumentu (odpowiadający dziedzinie abstrakcyjnej funkcji matematycznej), i jaki jest typ rezultatu
(odpowiadający przeciwdziedzinie abstrakcyjnej funkcji matematycznej), a przy tym kojarzy się łatwo
z pojęciami przekształcania i przyporządkowywania. To może tak? (tego też nie wklejaj do Leana, to
tylko ilustracja):
...
-- Można to zapisać krócej, bo *domyślnie* `+` oznacza w Leanie dodawanie liczb naturalnych, a więc Lean
-- może wywnioskować, że w definicji ...
def dodaj2' := fun n => n + 2
-- parametr `n` musi mieć typ `Nat`, wyrażenie `n + 2` w ciele funkcji musi mieć typ `Nat`, a więc stała
-- `dodaj2'` musi mieć typ `Nat → Nat`. Na razie jednak nie będziemy korzystać z tego udogodnienia.
```
...
W praktyce często używa się tych dwóch terminów wymiennie, przy czym częściej pojawia się termin
argument (np. "ta funkcja ma argument liczbowy" zamiast "ta funkcja ma parametr liczbowy"), ponieważ
zwykle można łatwo wywnioskować z kontekstu, czy chodzi o parametr-własność-funkcji, czy o
argument-własność-aplikacji (gdy mówimy "ta funkcja ma argument liczbowy" ewidentnie chodzi tak
naprawdę o parametr).
...
-- Ponieważ uzyskujemy w ten sposób pewną aplikację, trzeci krok to również ewaluacja aplikacji, a więc
...
zamiast *x* jest *1*, a cała początkowa aplikacja *(λ x . (λ y . x + y)) 1 2* zostaje zredukowana do
postaci:
...
gdy już powiem Ci coś o *klasach typów*) wiele definicji (dla różnych typów liczbowych, takich jak
liczby naturalne, całkowite, wymierne i rzeczywiste) i widoczna poniżej, zbyt skrótowa wersja nie
pozwala Leanowi wywnioskować, którą definicję wybrać:
...
Czy widzisz na tych przykładach, jak zapisywanie parametrów w nawiasach zaraz po nazwie definiowanej
...
musiała mieć typ `Nat → Nat` (już wiesz, dlaczego ta stała będzie miała ten typ, prawda?):
...
-- argument. Mówiąc dokładniej, ta aplikacja jest funkcją powstającą przez podstawienie `3` pod zmienną
-- `pierwsza` w "wewnętrznej" funkcji ...
-- `fun (druga : Nat) => pierwsza + druga`
-- ... czyli jest funkcją:
-- `fun (druga : Nat) => 3 + druga`
...
[^1]: Pierwszym znanym mi tak młodym, bo czternastoletnim (sic!) czytelnikiem tej książki był Szymon
    ...
    właśnie ten, dotyczący funkcji `plus`. Gdy jednak Szymon doczytał trochę dalej, wszystko się
